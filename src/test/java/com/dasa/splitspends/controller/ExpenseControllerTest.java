...
public class ExpenseControllerTest extends ControllerTestBase {
	...
	MvcResult result = mvc.perform(post("/expenses").header("Authorization", "Bearer " + jwt)
		.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(groupTestExpense)))
	...
	MvcResult result = mvc.perform(post("/expenses").header("Authorization", "Bearer " + jwt)
		.contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(nonGroupTestExpense)))
		.andExpect(status().isOk()).andReturn();
	Expense createdTestExpense = objectMapper.readValue(result.getResponse().getContentAsString(), Expense.class);
	result = mvc
		.perform(get("/expenses/{id}", createdTestExpense.getId()).header("Authorization", "Bearer " + jwt)
	...
	mvc.perform(delete("/expenses/{expenseId}", "9004").header("Authorization", "Bearer " + jwt)
		.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn();
	mvc.perform(get("/expenses/{id}", "9004").header("Authorization", "Bearer " + jwt)
